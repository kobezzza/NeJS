...
exports.compile = function (text) {
	text = this.escaper.replace('var __NEJS_THIS__ = this;\n' + text, true);
	var i = 1;
	...
	while (i) {
		...
		text.replace(arrowRgxp, function (sstr, $1, $2, pos, str) {
			i++;
			...
			text = str.substring(0, pos) + res + str.substring(pos + sstr.length);
		});
	}

	// Параметры функции
	text = text.replace(/(?:^([ \t]*).*?|^)(?:function|get|set)\s+[\w$]*\(([\s\S]*?)\)\s*{("__NEJS__";|)/gm,
		function (sstr, $1, $2, $3) {
			var restRgxp = /\.\.\./,
				defRgxp = /=/;
	...
	return this.escaper.paste(text.replace(/"__NEJS__";/gm, ''));
};