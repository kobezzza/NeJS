...
exports.compile = function (src) {
	src = this.replaceDangerBlocks('var __NEJS_THIS__ = this;\n' + src);
	var i = 1;
	...
	while (i) {
		...
		src.replace(arrowRgxp, function (sstr, $1, $2, pos, str) {
			i++;
			...
			for (var j = pos + sstr.length; j < str.length; j++) {
				...
				if (/[$\w\s]/.test(el)) {
					...
					} else if (word === 'function' || ((word === 'get' || word === 'set') && /[$\w]/.test(el))) {
						inFunc++;
						inFuncBOpen = bOpen;
					}
	...
	// Параметры функции
	src = src.replace(/(?:^([ \t]*).*?|^)(?:function|get|set)\s+[\w$]*\(([\s\S]*?)\)\s*{("__NEJS__";|)/gm,
		function (sstr, $1, $2, $3) {
			var restRgxp = /\.\.\./,
				defRgxp = /=/;
	...
	return this.pasteDangerBlocks(src.replace(/"__NEJS__";/gm, ''));
};